...
export class ColoringTool implements Tool {
  ...
  renderSettings(container: HTMLElement): void {
    ...
    container.appendChild(createImageDropZone('原画', 1));
  ...
  async executeColoring(context: ToolContext): Promise<void> {
    ...
    const folderId = await createCacheFolder(`ColoringSetup_${Date.now()}`);

    // ① 透明レイヤー追加前入力
    await context.cacheResult(originalItem.file, this.id, { name: '1_透明レイヤー追加前入力.png', folderId });
    ...
    showToast(`アスペクト比を ${bestAr} に自動選択しました。`, 'info');
    ...
    await context.cacheResult(paddedBlob, this.id, { name: '2_透明レイヤー追加後入力.png', folderId });
    ...
    try {
      ...
      await context.cacheResult(geminiBlob, this.id, { name: `3_Gemini生成後クロップ前出力${ext}`, folderId });
      ...
      if (finalBlob) {
        await context.cacheResult(finalBlob, this.id, { name: `4_クロップ後出力.png`, folderId });
